    ...
    if (!empty($image)) {
        if (in_array($imageType, $allowedImageType)) {
            if (move_uploaded_file($tempPath, $originalPath)) {
                $insqry = "UPDATE tbl_teacher SET teacher_name= '$name', teacher_contact=$contact, teacher_photo='$image' WHERE teacher_id='" . $_SESSION["teacherid"] . "'";
                if (mysqli_query($con, $insqry)) {
                    $_SESSION["teachername"] = $name;
                    $_SESSION["teacherpic"] = $image;
...
<head>
    ...
    <title>Teacher | EditProfile</title>
                        ...
                        <div class="table-responsive">
                            <table class="table table-primary table-bordered table-hover">
                                <tbody>
                                    ...
                                    <tr>
                                        <th>Name</th>
                                        <td><input type="text" class="form-control" name="name" value="<?php echo $_SESSION["teachername"]; ?>" required pattern="^[A-Z]+[a-zA-Z ]*$" title="Name Allows Only Alphabets,Spaces and First Letter Must Be Capital Letter"></td>
                                    </tr>
                                    <tr>
                                        <th>Email</th>
                                    ...
                                    <tr>
                                        <th>Contact</th>
                                        <td><input type="text" class="form-control" name="contact" value="<?php echo $row["teacher_contact"]; ?>" maxlength=10 required pattern="[7-9]{1}[0-9]{9}" title="Phone number with 7-9 and remaing 9 digit with 0-9"></td>
                                    </tr>
                                    <tr>
                                        <th>Photo</th>
                                    ...
                                    <tr>
                                        <td colspan=2 align="center"><button type="submit" class="btn btn-primary mr-2" name="btn_submit">Update Profile</button>
