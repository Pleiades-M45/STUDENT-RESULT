    ...
    if (!empty($image)) {
        if (in_array($imageType, $allowedImageType)) {
            if (move_uploaded_file($tempPath, $originalPath)) {
                $insqry = "UPDATE tbl_admin SET admin_name= '$name', admin_photo='$image' WHERE admin_id='" . $_SESSION["adminid"] . "'";
                if (mysqli_query($con, $insqry)) {
                    $_SESSION["adminname"] = $name;
                    $_SESSION["adminpic"] = $image;
?>
                    <script>
                                ...
                                <tbody>
                                    <tr>
                                        <th colspan=2 id="preview" style="vertical-align:middle;text-align:center;">
                                            <img src="../Assets/Files/<?php echo $row["admin_photo"]; ?>" width="200" height="200">
                                        </th>
                                    </tr>
                                    <tr>
                                        <th>Name</th>
                                        <td><input type="text" class="form-control" name="name" value="<?php echo $_SESSION["adminname"]; ?>" pattern="^[A-Z]+[a-zA-Z ]*$" title="Name Allows Only Alphabets,Spaces and First Letter Must Be Capital Letter" required></td>
                                    </tr>
                                    <tr>
                                        <th>Email</th>
