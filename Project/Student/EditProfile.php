...
ob_start();
include('Head.php');
$selQry = "SELECT * FROM tbl_student WHERE student_id='" . $_SESSION["studentid"] . "'";
    ...
    if (!empty($image)) {
        if (in_array($imageType, $allowedImageType)) {
            if (move_uploaded_file($tempPath, $originalPath)) {
                $insqry = "UPDATE tbl_student SET student_name= '$name', student_contact=$contact,student_photo='$image' WHERE student_id='" . $_SESSION["studentid"] . "'";
                if (mysqli_query($con, $insqry)) {
                    $_SESSION["studentname"] = $name;
                    $_SESSION["studentpic"] = $image;
?>
                    <script>
                        ...
                        <div class="table-responsive">
                            <table class="table table-primary table-bordered table-hover">
                                <tbody>
                                    ...
                                    <tr>
                                        <th>Name</th>
                                        <td><input type="text" class="form-control" name="name" value="<?php echo $_SESSION["studentname"]; ?>" required pattern="^[A-Z]+[a-zA-Z ]*$" title="Name Allows Only Alphabets,Spaces and First Letter Must Be Capital Letter"></td>
                                    </tr>
                                    <tr>
                                        <th>Email</th>
                                    ...
                                    <tr>
                                        <th>Contact</th>
                                        <td><input type="text" class="form-control" name="contact" value="<?php echo $row["student_contact"]; ?>" maxlength=10 required pattern="[7-9]{1}[0-9]{9}" title="Phone number with 7-9 and remaing 9 digit with 0-9"></td>
                                    </tr>
                                    <tr>
                                        <th>Photo</th>
